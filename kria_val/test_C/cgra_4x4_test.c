...
#define GPIO9_BASE  0xA00E0000
#define GPIO10_BASE 0xA00F0000
#define GPIO11_BASE 0xA0100000
#define GPIO12_BASE 0xA0110000
...
volatile uint32_t *g0;
volatile uint32_t *g1,*g2,*g3,*g4,*g5,*g6,*g7,*g8;
volatile uint32_t *g9,*g10,*g11,*g12;
...
void write_inj_data()
{
    ...
    write_channel(g5,1,inj_data_flat[9]);

    write_channel(g6,0,inj_data_flat[10]);
    write_channel(g6,1,inj_data_flat[11]);

    write_channel(g7,0,inj_data_flat[12]);
    write_channel(g7,1,inj_data_flat[13]);

    write_channel(g8,0,inj_data_flat[14]);
    write_channel(g8,1,inj_data_flat[15]);
}

void write_sel()
{
    write_channel(g9,0,sel_flat[0]);
    write_channel(g9,1,sel_flat[1]);

    write_channel(g10,0,sel_flat[2]);
    write_channel(g10,1,sel_flat[3]);

    write_channel(g11,0,sel_flat[4]);
    write_channel(g11,1,sel_flat[5]);

    write_channel(g12,0,sel_flat[6]);
    write_channel(g12,1,sel_flat[7]);
}
...
void print_pe_outputs()
{
    ...
    out[0]  = read_channel(g1,1);
    out[1]  = read_channel(g1,0);

    out[2]  = read_channel(g2,1);
    out[3]  = read_channel(g2,0);

    out[4]  = read_channel(g3,1);
    out[5]  = read_channel(g3,0);

    out[6]  = read_channel(g4,1);
    out[7]  = read_channel(g4,0);

    out[8]  = read_channel(g5,1);
    out[9]  = read_channel(g5,0);

    out[10] = read_channel(g6,1);
    out[11] = read_channel(g6,0);

    out[12] = read_channel(g7,1);
    out[13] = read_channel(g7,0);

    out[14] = read_channel(g8,1);
    out[15] = read_channel(g8,0);
