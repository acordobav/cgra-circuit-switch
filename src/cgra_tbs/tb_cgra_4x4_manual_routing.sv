...
module tb_cgra_4x4_manual_routing;
...
initial begin
	...
	for (int r = 0; r < ROWS_P; r++) begin
		wait_for_value_at(r, COLS_P-1, payload[r]);
		$display("[PASS] Route row %0d: ( %0d,0 ) -> ( %0d,3 ) payload=%h",
				 r, r, r, payload[r]);
	end

	$display("[DONE] All 4 routes configured and verified concurrently.");
	$finish;
end
